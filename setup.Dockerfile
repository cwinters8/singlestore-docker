...
# script to refresh host rsa keys
COPY ./rsa_refresh.sh /root/

# deploy SingleStoreDB
ENTRYPOINT /root/rsa_refresh.sh && sdb-deploy setup-cluster -i /root/.ssh/singlestorekey.pem --license ${LICENSE} \
  --master-host ubuntu@${MASTER_HOST} --aggregator-hosts ubuntu@${CHILD_HOST} \
  --leaf-hosts ubuntu@${LEAF1_HOST},ubuntu@${LEAF2_HOST} --password ${PASSWORD} -v -y
